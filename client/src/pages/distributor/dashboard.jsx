...
export function DistributorDashboard() {
  ...
  const {
    ...
    transactionsData.forEach((t) => {
      ...
      if (t.status === 'FAILED') {
        ...
        tSuccess++;
        tSuccessAmount += t.distributorAmount || (t.amount * 0.45); // use distributorAmount
      }
    });
    ...
    let cDistributors = 0; // Not applicable for distributor dashboard directly unless needed
  ...
  useEffect(() => {
    if (user?._id) {
      setLoading(true);
      axios
        .get(`${PAYMENT_URL}/transaction/find?distributor=${user._id}`)
        .then(res => {
  ...
  return (
    <div className="backgroundDiv min-h-screen">
      <div className="container ">
        <div className="header py-4 bg-black border-b border-gray-700 text-white font-bold pl-2">Distributor Dashboard</div>
        <br />
        {loading ? <Loader /> : (
